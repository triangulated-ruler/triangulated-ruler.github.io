...
#def[
	We define an object $Z in pStk$ to be a scheme if the following condition holds: 
	+	$Z$ satisfies Zariski descent. 
	+	The diagonal $Delta:Z->Z times Z$ is affine schematic, and for every $T in (Aff)_(\/Z times Z)$, we have the induced map $(T times_(Z times Z)Z)_cl->T_cl$ a closed immersion. 
	+	There is a family of affine schemes $f_i:S_i->Z$ such that each $f_i$ is an open embedding, and the family is jointly surjective, i.e., $(f_i)_cl:(S_i)_cl->Z_cl$ is jointly surjective. 
] <Def_sch>

#def[
	We then inductively define the notion of $k$-Artin stacks. 
	+	We define an $0$-Artin stack to be 
] <Def_Art_stk>
...
#prop[
	The functor $QCoh^*_Aff:Aff^op->DGCat$ satisfies flat descent. Hence, it satisfies ppf, etale, and Zariski descent. 
] <Prop_QCoh_aff_flat_desc>
#proof[
	Apply the standard faithfully flat descent and the technique of the proof in @Lem_flat_subcanonical. 
]
...
#const[
	...
	In particular, by our construction we have $f^*$ is right $t$-exact and $f_*$ is left $t$-exact. 
] <Const_t_str_QCoh>

#coro[
	$QCoh^(*,<=0)$ satisfies flat descent. 
] <Coro_QCoh_t_str_conn_flat_desc>
#proof[
	By construction. 
]

#lem[
	$QCoh^(*,>=0):(pStk^(k#[-Artin]))^op->DGCat$ satisfies flat descent. 
] <Lem_QCoh_t_str_coconn_flat_desc>

#lem[
	Let $f:X->Y$ be a map of schemes almost of finite type. Then, we have the pushforward $f_*:QCoh(X)->QCoh(Y)$ of finite tor-amplitude. 
] <Lem_QCoh_f_usual_pshfwd_finite_tor_amplitude_aft>
#proof[
	We have already that $f_*$ is left $t$-exact. Now, for $cal(F) in QCoh(X)^(<=0)$, we prove $f_*(cal(F)) in QCoh(Y)^(<=N)$ for some $N$. Firstly, we take $U subset Y$ an affine open, and take $V=U times_Y X$. Then, we have $Gamma(U,f_*(cal(F)))=Gamma(V,cal(F))$. We apply the cohomological devissage to compute this. Namely, we consider the spectral sequence associated to the Postnikov filtration of $cal(F)$. The graded pieces are given by 
	$Gamma(V,cal(H)^q (cal(F)))=Gamma(V_cl,cal(H)^q (cal(F)))$, so we obtain a spectral sequence 
	$ E_2^(p,q)=H^p (V_cl,cal(H)^q (cal(F)))==>H^(p+q)(Gamma(V,cal(F))) $
	Now, since $cal(F) in QCoh(X)^(<=0)$, $cal(H)^q (cal(F))=0$ for $q>0$, and since $X$ is almost of finite type, $V_cl$ is a finite dimensional Noetherian topological space, say of dimension $d$. Hence, $H^p (V_cl,cal(G))=0$ for all $p>d$. Hence, taking $N=d$, we have $H^n (Gamma(V,F))=0$ for all $n>N$, hence $f_*(cal(F))in QCoh(Y)^(<=d)$, where $d=dim Y_cl$. 
]
...
#lem[
	Let $f:X->Y$ be a morphism in $Sch_aft$. Then, $f_*^(Ind(Coh))$ is right $t$-exact up to a shift, hence is of finite tor-amplitude. 
] <Lem_IndCoh_Sch_aft_star_pshfwd_fin_tor_amplitude>
#proof[
	This follows from @Lem_QCoh_f_usual_pshfwd_finite_tor_amplitude_aft
]
...
The construction of the full functorial structure is not direct because we need to specify the higher structures. However, we do not need to determine them by force, but via a clever construction of an auxiliary $oo$-category. We will use similar techniques to obtain further functorialities of $Ind(Coh)$. 
...
#proof[
	...
	is an equivalence. Again, since all functors are continuous, we may assume $cal(F) in Coh(X)$, and again both sides are in $Ind(Coh)(X)^+$, so we may apply $Psi_X$ to both sides. In this case, it reduces to the Zariski descent of quasi-coherent sheaves, which is by @Prop_QCoh_aff_flat_desc. 
]
...
#proof[
	For only if, we have $j^(Ind(Coh),*)$ $t$-exact by @Coro_IndCoh_star_pullback_open_t_exact. For the other direction, assume $f^(Ind(Coh),*)(cal(F))in refl(Ind(Coh)(U))^(<=0)$, apply $Psi_U$, and we obtain the result from @Prop_QCoh_aff_flat_desc. Assume $f^(Ind(Coh),*)(cal(F))in refl(Ind(Coh)(U))^(>=0)$, the assertion follows from the construction of the inverse functor as in @Prop_IndCoh_Sch_aft_Zariski_desc. 
]
...
#def[
	The above shows that for a proper map $f:X->Y$, we have $f_*^(Ind(Coh)):Ind(Coh)(X)->Ind(Coh)(Y)$ is continuous, and its right adjoint preserves filtered colimits, hence preserves all colimits. Thus, $f_*^(Ind(Coh))$ admit a continuous right adjoint $f^!:Ind(Coh)(Y)->Ind(Coh)(X)$. This is a functor of $QCoh(Y)$-modules. 

	By the general formalism of passing to right adjoints, we can define a functor 
	$ Ind(Coh)^!_((Sch_aft)_(#[proper])):(Sch_aft)_(#[proper])^op->DGCat $
] <Def_exceptional_pullback_Sch_aft_proper>

By @Prop_bc_QCoh_sch_qc, the functor $f_*$ admit a discontinuous right adjoint $f^(!,QCoh)$. 

#lem[
	The square 
	// https://q.uiver.app/#r=typst&q=WzAsNCxbMCwwLCJyZWZsKEluZChDb2gpKFgpKV4rIl0sWzEsMCwicmVmbChJbmQoQ29oKShZKSleKyJdLFswLDEsIlFDb2goWCleKyJdLFsxLDEsIlFDb2goWSleKyJdLFsxLDAsImZeISJdLFszLDIsImZeKFFDb2gsISkiXSxbMCwyLCJQc2lfWCIsMl0sWzEsMywiUHNpX1kiXV0=
	#align(center, diagram({
		node((-1, 0), [$refl(Ind(Coh)(X))^+$])
		node((0, 0), [$refl(Ind(Coh)(Y))^+$])
		node((-1, 1), [$QCoh(X)^+$])
		node((0, 1), [$QCoh(Y)^+$])
		edge((0, 0), (-1, 0), [$f^!$], label-side: right, "->")
		edge((0, 1), (-1, 1), [$f^(QCoh,!)$], label-side: left, "->")
		edge((-1, 0), (-1, 1), [$Psi_X$], label-side: right, "->")
		edge((0, 0), (0, 1), [$Psi_Y$], label-side: left, "->")
	}))
	commutes. 
] <Lem_exceptional_pullback_agree_IndCoh_QCoh_ev_coconn>
#proof[
	Since $Psi_X$ and $Psi_Y$ are equivalences, and the functors $f_*^(Ind(Coh))$ and $f_*$ are intertwined by $Psi$. 
]

The analogous fact is *not* true if we remove the eventually coconnectivity condition. 

#prop[
	Let // https://q.uiver.app/#r=typst&q=WzAsNCxbMCwwLCJYXzEiXSxbMSwwLCJYXzIiXSxbMCwxLCJZXzEiXSxbMSwxLCJZXzIiXSxbMCwxLCJnX1giXSxbMiwzLCJnX1kiXSxbMCwyLCJmXzEiLDJdLFsxLDMsImZfMiJdXQ==
	#align(center, diagram({
		node((0, 0), [$X_1$])
		node((1, 0), [$X_2$])
		node((0, 1), [$Y_1$])
		node((1, 1), [$Y_2$])
		edge((0, 0), (1, 0), [$g_X$], label-side: left, "->")
		edge((0, 1), (1, 1), [$g_Y$], label-side: left, "->")
		edge((0, 0), (0, 1), [$f_1$], label-side: right, "->")
		edge((1, 0), (1, 1), [$f_2$], label-side: left, "->")
		node((0.2, 0.2), [$Cartmark$])
	}))
	be a Cartesian diagram where $f_i$ are proper maps. Then, we have the base change isomorphism 
	$ (g_X)_*^(Ind(Coh))compose f_1^! tilde.eq f_2^! compose (g_Y)_*^(Ind(Coh)) $
] <Prop_IndCoh_base_change_proper_exceptional_pullback>
#proof[
	Note all functors involved are continuous, so it suffices to verify the equivalence for the compact objects $cal(F) in Coh(Y_1)$. Then, the functors have images in the following diagram 
	// https://q.uiver.app/#r=typst&q=WzAsNCxbMCwwLCJJbmQoQ29oKShYXzEpXisiXSxbMSwwLCJJbmQoQ29oKShYXzIpXisiXSxbMCwxLCJJbmQoQ29oKShZXzEpXisiXSxbMSwxLCJJbmQoQ29oKShZXzIpXisiXSxbMCwxLCIoZ19YKV8qXihJbmQoQ29oKSkiXSxbMiwzLCIoZ19ZKV8qXihJbmQoQ29oKSkiXSxbMiwwLCJmXzFeISJdLFszLDEsImZfMl4hIiwyXV0=
	#align(center, diagram({
		node((0, 0), [$Ind(Coh)(X_1)^+$])
		node((1, 0), [$Ind(Coh)(X_2)^+$])
		node((0, 1), [$Ind(Coh)(Y_1)^+$])
		node((1, 1), [$Ind(Coh)(Y_2)^+$])
		edge((0, 0), (1, 0), [$(g_X)_*^(Ind(Coh))$], label-side: left, "->")
		edge((0, 1), (1, 1), [$(g_Y)_*^(Ind(Coh))$], label-side: left, "->")
		edge((0, 1), (0, 0), [$f_1^!$], label-side: left, "->")
		edge((1, 1), (1, 0), [$f_2^!$], label-side: right, "->")
	}))
	To prove the above diagram commute, we first apply the equivalence $Psi:Ind(Coh)^+->QCoh^+$. Then, the equivalence follows from the base change for $QCoh$ and passing to right adjoints 
	// https://q.uiver.app/#r=typst&q=
	#align(center, diagram({
		node((0, 0), [$QCoh(X_1)^+$])
		node((1, 0), [$QCoh(X_2)^+$])
		node((0, 1), [$QCoh(Y_1)^+$])
		...
		node((-2, 0), [$QCoh(X_1)^+$])
		node((-1, 0), [$QCoh(X_2)^+$])
		edge((0, 0), (1, 0), [$(g_X)_*$], label-side: left, "->")
		edge((0, 1), (1, 1), [$(g_Y)_*$], label-side: left, "->")
		edge((0, 1), (0, 0), [$f_1^(QCoh,!)$], label-side: left, "->")
		edge((1, 1), (1, 0), [$f_2^(QCoh,!)$], label-side: right, "->")
		edge((-1, 0), (-2, 0), [$g_X^*$], label-side: right, "->")
		edge((-1, 1), (-2, 1), [$g_Y^*$], label-side: right, "->")
		edge((-2, 0), (-2, 1), [$(f_1)_*$], label-side: right, "->")
		edge((-1, 0), (-1, 1), [$(f_2)_*$], label-side: left, "->")
		edge((-0.6, 0.5), (-0.4, 0.5), [$(-)^R$], label-side: left, "~>")
	}))
]
...
#prop[
	...
	be a Cartesian diagram where $f_i$ are proper and $g_X$, $g_Y$ are eventually coconnective. Then, by the $(f_*^(Ind(Coh)),f^!)$-adjunction and the $(*,*)$-base change for $Ind(Coh)$ (@Prop_IndCoh_Sch_aft_base_change), we have a map 
	$ g_X^(Ind(Coh),*)compose f_2^!tilde.eq f_1^! compose g_Y^(Ind(Coh),*) $
	The above map is an equivalence. 
]
#proof[
	Firstly, since both functors commutes with filtered colimits, it suffices to prove the equivalence for $cal(F)in Coh(Y_2)$. Note that $f_i^!$ are left $t$-exact up to a finite shift by passing to its left adjoint and apply @Lem_IndCoh_Sch_aft_star_pshfwd_fin_tor_amplitude. Thus, since $g_X^(Ind(Coh),*)$ is also left $t$-exact, it suffices to prove the diagram // https://q.uiver.app/#r=typst&q=
	#align(center, diagram({
		node((0, 0), [$refl(Ind(Coh)(X_1))^+$])
		node((1, 0), [$refl(Ind(Coh)(X_2))^+$])
		node((0, 1), [$refl(Ind(Coh)(Y_1))^+$])
		node((1, 1), [$refl(Ind(Coh)(Y_2))^+$])
		edge((0, 1), (0, 0), [$f_1^!$], label-side: left, "->")
		edge((1, 1), (1, 0), [$f_2^!$], label-side: right, "->")
		edge((1, 0), (0, 0), [$g_X^(*,Ind(Coh))$], label-side: right, "->")
		edge((1, 1), (0, 1), [$g_Y^(*,Ind(Coh))$], label-side: right, "->")
	}))
	commutes. 

	Notice that the equivalence is Zariski local on $Y_2$, so we may assume that $g_X$ and $g_Y$ are affine. Since for $f$ affine we have $f_*$ is monadic, in particular it is conservative. Hence, it suffices to prove that 
	$ (g_X)^(Ind(Coh))_* compose g_X^(Ind(Coh),*)compose f_2^! tilde.eq (g_X)_*^(Ind(Coh))compose f_1^! compose g_Y^(Ind(Coh),*) $
	By @Coro_proj_formula_QCoh_IndCoh_ev_coconn, we have $(g_X)^(Ind(Coh))_* compose g_X^(Ind(Coh),*)tilde.eq g_(X,*)(cal(O)_X_1)times.o-$. But by definition, 
	$ g_(X,*)(cal(O)_X_1)=g_(X,*)compose f_1^*(cal(O)_Y_1)=f_2^* compose g_(Y,*)(cal(O)_Y_1) $
	by @Prop_bc_QCoh_sch_qc. 
	
	Hence, we have 
	$ (g_X)^(Ind(Coh))_* compose g_X^(Ind(Coh),*)compose f_2^!(cal(F))=f_2^*(g_(Y,*)(cal(O)_Y_1))times.o f_2^!(cal(F))=f_2^!(g_(Y,*)(cal(O)_Y_1)times.o cal(F)) $
	by linearity of $f_2^!$ as $QCoh$-modules. 

	On the other hand, we have 
	$ (g_X)_*^(Ind(Coh))compose f_1^! compose g_Y^(Ind(Coh),*)(cal(F))=f_2^! compose (g_Y)_*^(Ind(Coh)) compose g_Y^(Ind(Coh),*)(cal(F))=f_2^!(g_(Y,*)(cal(O)_Y_1)times.o cal(F)) $
	and unwinding definitions we obtain that the comparison map between this two is given by the identity. 
]

Then, we temporarily set $CAlg=CAlg(Sp)$, and give some descent results. 
